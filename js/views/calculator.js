...
BS.Views.calculator = {
    ...
    return `
      <section class="panel">
        <div class="panel-head">
          <div>
            <div class="panel-title">倉位計算器</div>
            <div class="panel-subtitle">
              本金 ${BS.Config.balance} U · 單筆風險 ${BS.Config.riskPct}%
            </div>
          </div>
        </div>
  ...
  bind() {
    ...
    if (saveButton) {
      saveButton.addEventListener("click", () => {
        ...
        const rawEntry = form.elements.entry.value.trim();

        BS.TradeRecords.add({
          ...result.data,
          entryDecimals:
            BS.RiskCalculator.decimalPlaces(rawEntry)
        });
  ...
  captureState(form) {
    this.state = {
      ...
      entry: form.elements.entry.value.trim(),
      sl: form.elements.sl.value.trim(),
      leverage: Number(form.elements.leverage.value)
    };
  },

  calculate(form) {
    const result = BS.RiskCalculator.calculate({
    ...
    const margin = document.getElementById("marginPctResult");
    const notional = document.getElementById("notionalResult");
    const saveButton = document.getElementById("saveTradeBtn");
    ...
    if (margin) {
      margin.textContent =
        result.data && Number.isFinite(result.data.marginPct)
          ? `${result.data.marginPct.toFixed(2)}%`
          : "—";
    }
